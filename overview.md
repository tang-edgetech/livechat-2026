...
### 6.1 Settings tabs
General · System · Automation · Bot · Integration · Users · **Merchants** · Audit Logs (Admin/Super Admin only) · Files
...
- The "AI"/third-party step is one of those action types (`call_integration`) — a **generic outbound REST call**, not a hardcoded vendor SDK. `bot_flow` holds an `integration_id` (nullable FK → `integration`, type `webhook`) that supplies the endpoint URL + auth secret; when that node runs, the Go backend POSTs the conversation context to the endpoint and posts the response back into the chat as a `bot` message. Swapping or adding a provider later is just adding a new `integration` row, no flow-engine changes.
